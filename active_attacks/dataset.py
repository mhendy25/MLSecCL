...
class SafetyDataset(Dataset):
    def __init__(self, tokenizer, instruction_file, reweighting=False) -> None:
        super().__init__()
        self.tokenizer = tokenizer
...
def get_dataloader(name, tokenizer, prompt_file, sft_file=None,  batch_size=16, shuffle=True, reweighting=False):
    if name == "gfn":
        ...
        tr_dataset = SafetyDataset(tokenizer, prompt_file, reweighting=reweighting)
        if reweighting:
            tr_dataloader = DataLoader(
                tr_dataset, batch_size, collate_fn=DataCollatorForSeq2Seq(tokenizer),
                sampler=WeightedRandomSampler(tr_dataset.weights, num_samples=len(tr_dataset), replacement=True))
        else:
            tr_dataloader = DataLoader(
                tr_dataset, batch_size, shuffle=shuffle, collate_fn=DataCollatorForSeq2Seq(tokenizer))
